  ...
  if (cfg.map) {
    ...
    for (var p in cfg.map) {
      ...
      if (typeof v !== 'string') {
        ...
        // if a package main.ts, revert it
        var pkgMatch = '';
 ...
 * SystemJS.packages = {
 *   jquery: {
 *     main.ts: 'index.js', // when not set, package name is requested directly
 *     format: 'amd',
 ...
 *
 * Detailed Behaviours
 * - main.ts can have a leading "./" can be added optionally
 * - map and defaultExtension are applied to the main.ts
 ...
 *
 *   // will first request 'packages/test/package.json' before the main.ts
...
(function() {
  ...
  function applyPackageConfigSync(loader, pkg, pkgName, subPath, skipExtensions) {
    ...
    if (!subPath) {
      ...
      // also no submap if name is package itself (import 'pkg' -> 'path/to/pkg.js')
        ...
        // NB can add a default package main.ts convention here when defaultJSExtensions is deprecated
  ...
  function doMapSync(loader, pkg, pkgName, mapMatch, path, skipExtensions) {
    ...
    // package map to main.ts / base-level
    if (mapped == '.')
      mapped = pkgName;
  ...
  function loadPackageConfigPath(loader, pkgName, pkgConfigPath) {
    ...
    .then(function() {
      ...
      // main.ts object becomes main.ts map
      if (typeof pkg.main == 'object') {
        pkg.map = pkg.map || {};
        pkg.map['./@main.ts'] = pkg.main;
        pkg.main['default'] = pkg.main['default'] || './';
      ...
      .then(function(address) {
        ...
        if (pkgName) {
          ...
          if (pkg.meta) {
            ...
            // NB support a main.ts shorthand in meta here?
            getMetaMatches(pkg.meta, subPath, function(metaPattern, matchMeta, matchDepth) {
